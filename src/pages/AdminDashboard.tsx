  ...
  if (!user || user.role !== 'admin') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-white font-bengali px-4 text-center">
        ...
        <h1 className="text-2xl font-black">প্রাইভেট এলাকা! (Access Denied)</h1>
        <p className="text-xs text-slate-400 mt-2 max-w-sm leading-relaxed">
          এই পেজটি শুধুমাত্র বাংলাদেশ সার্ভিস ডিরেক্টরির সুপার অ্যাডমিনদের জন্য সুরক্ষিত। সাধারণ ব্যবহারকারীদের প্রবেশ সংরক্ষিত।
        </p>
        <div className="flex gap-3 mt-6">
          <button onClick={() => navigate('/')} className="bg-slate-800 hover:bg-slate-700 text-white px-6 py-2.5 rounded-full text-xs font-bold border border-slate-700">
            হোমে ফিরে যান
          </button>
          <button onClick={() => navigate('/login')} className="bg-[#006A4E] hover:bg-[#005841] text-white px-6 py-2.5 rounded-full text-xs font-bold shadow-lg">
            অ্যাডমিন অ্যাকাউন্ট দিয়ে লগইন
          </button>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-900 text-slate-100 font-bengali flex flex-col md:flex-row">
        ...
        <aside className="w-full md:w-64 bg-slate-950 p-6 border-b md:border-b-0 md:border-r border-slate-800 flex flex-col justify-between">
          <div className="space-y-6">
            ...
            <nav className="space-y-1.5 text-xs font-semibold pt-4">
              <button onClick={() => setActiveTab('overview')} className={`w-full flex items-center gap-2.5 px-4 py-3 rounded-xl transition ${activeTab === 'overview' ? 'bg-[#006A4E] text-white font-bold' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}>
                <BarChart3 className="w-4 h-4" /><span>ড্যাশবোর্ড সামারি</span>
              </button>

              <button onClick={() => setActiveTab('services')} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition ${activeTab === 'services' ? 'bg-[#006A4E] text-white font-bold' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}>
                <div className="flex items-center gap-2.5"><Grid className="w-4 h-4" /><span>সেবা ব্যবস্থাপনা</span></div>
                <span className="bg-slate-800 text-emerald-300 text-[10px] px-2 py-0.5 rounded-full">{totalServices}</span>
              </button>

              <button onClick={() => setActiveTab('reports')} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition ${activeTab === 'reports' ? 'bg-[#006A4E] text-white font-bold' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}>
                <div className="flex items-center gap-2.5"><AlertTriangle className="w-4 h-4" /><span>লিংক রিপোর্ট</span></div>
                {pendingReportsCount > 0 && <span className="bg-rose-500/20 text-rose-300 text-[10px] px-2 py-0.5 rounded-full border border-rose-500/30">{pendingReportsCount}</span>}
              </button>

              <button onClick={() => setActiveTab('users')} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition ${activeTab === 'users' ? 'bg-[#006A4E] text-white font-bold' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}>
                <div className="flex items-center gap-2.5"><UserCog className="w-4 h-4" /><span>ইউজার ও অ্যাডমিন</span></div>
          ...
          <div className="pt-6 border-t border-slate-800 space-y-2">
            <Link to="/" className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-slate-300 font-bold py-2.5 rounded-xl text-xs transition border border-slate-800">
              <ArrowLeft className="w-4 h-4" /><span>মূল ওয়েবসাইটে যান</span>
            </Link>
            <button onClick={() => { logout(); navigate('/'); }} className="w-full flex items-center justify-center gap-2 bg-rose-950/50 hover:bg-rose-900 text-rose-300 font-bold py-2.5 rounded-xl text-xs transition border border-rose-900/50">
        ...
        <main className="flex-1 p-6 md:p-10 space-y-8 overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
            ...
            {activeTab === 'services' && (
              <button onClick={() => setIsAddModalOpen(true)} className="bg-[#006A4E] hover:bg-[#005841] text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow-lg transition flex items-center gap-2">
                <Plus className="w-4 h-4" /><span>নতুন সেবা যোগ করুন</span>
              </button>
            )}

            {activeTab === 'users' && (
              <button onClick={() => setIsAddUserModalOpen(true)} className="bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow-lg transition flex items-center gap-2">
                <UserPlus className="w-4 h-4" /><span>+ নতুন অ্যাডমিন / ইউজার যোগ করুন</span>
              </button>
            )}
          </div>
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between text-slate-400"><span className="text-xs font-semibold">মোট সেবা</span><Grid className="w-5 h-5 text-emerald-400" /></div>
                  <p className="text-3xl font-black text-white">{totalServices}</p>
                </div>
                <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between text-slate-400"><span className="text-xs font-semibold">ভেরিফায়েড</span><CheckCircle2 className="w-5 h-5 text-blue-400" /></div>
                  <p className="text-3xl font-black text-white">{verifiedCount}</p>
                </div>
                <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between text-slate-400"><span className="text-xs font-semibold">জনপ্রিয়</span><BarChart3 className="w-5 h-5 text-purple-400" /></div>
                  <p className="text-3xl font-black text-white">{popularCount}</p>
                </div>
                <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between text-slate-400"><span className="text-xs font-semibold">পেন্ডিং রিপোর্ট</span><AlertTriangle className="w-5 h-5 text-rose-400" /></div>
                  <p className="text-3xl font-black text-white">{pendingReportsCount}</p>
                </div>
                <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between text-slate-400"><span className="text-xs font-semibold">অ্যাডমিন সদস্য</span><Crown className="w-5 h-5 text-amber-400" /></div>
                  <p className="text-3xl font-black text-white">{adminCount}</p>
                </div>
                <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between text-slate-400"><span className="text-xs font-semibold">নাগরিক ইউজার</span><Users className="w-5 h-5 text-cyan-400" /></div>
                  ...
                  <tbody className="divide-y divide-slate-900 text-slate-300">
                    {filteredServices.map(s => (
                      <tr key={s.id} className="hover:bg-slate-900/50">
                        ...
                        <td className="p-3.5">
                          <button onClick={() => toggleVerifyService(s.id)} className={`px-2.5 py-1 rounded-full text-[10px] font-bold border transition ${s.verified ? 'bg-emerald-950 text-emerald-400 border-emerald-800 hover:bg-rose-950 hover:text-rose-400' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-emerald-950 hover:text-emerald-400'}`}>
                            {s.verified ? '✓ Verified' : '+ Verify'}
                          </button>
                        </td>
                        <td className="p-3.5">
                          <button onClick={() => { if (confirm(`"${s.nameBn}" মুছে ফেলতে চান?`)) deleteService(s.id); }} className="p-1.5 rounded bg-rose-950 hover:bg-rose-900 text-rose-400 transition" title="মুছে ফেলুন">
                  ...
                  {reports.map((r) => (
                    <div key={r.id} className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        {r.status === 'pending' && (
                          <button onClick={() => resolveReport(r.id)} className="bg-emerald-900 hover:bg-emerald-800 text-emerald-200 font-bold px-3 py-1.5 rounded-lg transition text-xs flex items-center gap-1">
                            <Check className="w-3.5 h-3.5" /> সমাধান
                          </button>
                        )}
                        <button onClick={() => deleteReport(r.id)} className="bg-slate-800 hover:bg-rose-950 text-slate-400 hover:text-rose-400 p-1.5 rounded-lg transition">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
